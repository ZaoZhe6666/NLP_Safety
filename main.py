...
# 参考链接 https://blog.csdn.net/kingsonyoung/article/details/90545746
...
def get_label_list(file_list):
    # 提取出标签名
    label_name_list = [file.split("/")[3] for file in file_list]
    # 标签名对应的数字
    label_list = []
...
if __name__ == "__main__":
    ...
    train_dir = "dataset/aclImdb/train"  # 训练集路径
    test_dir = "dataset/aclImdb/test"  # 测试集路径
    ...
    # 过程应该是 
    '''
        1. 预准备阶段：包括 停用表 + 词向量模型 + 词向量生成工具
        2. 数据准备阶段： 读入数据集，生成 DataLoader
        3. 获取模型
        4. 执行测试
    '''